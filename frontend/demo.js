...
const BidNow = () => {
  ...
  const handlePlaceBid = async (product) => {
    try {
      ...
      // Get sellerId from the product data
      const sellerId = product.userId; // Assuming userId in product data is the seller's ID
      ...
      const newBid = {
        ...
        bidderId: currentUser.uid, // BidderId
        productId, // Include the product ID to associate the bid with the product
      ...
      // Navigate to the bid section after placing the bid
